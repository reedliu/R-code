...
# check WGCNA power
WGCNA_matrix = t(RNAseq_voom[order(apply(RNAseq_voom,1,mad)[1:4000], decreasing = T),])
...
if(T){
    ...
    #abline(h = 45000, col = "red") #先画一条辅助线
    dev.off()
}
...
if(T){
    net = blockwiseModules(
        ...
        maxBlockSize = 5000, #设置值大于datExpr基因数
...
if(T){
    ...
    design=model.matrix(~0+ make.names(datTraits$source))
    colnames(design)=as.character(metadata$source_name_ch1) %>% unique()
    # 计算二者相关性
    ...
    if(T){
        ...
        dev.off()
        options(digits=3)
        cor_module <- "Actin_buffer_24hrs" #选择要计算相关性的表型
...
# 【图】各个模块基因显著性
if(T){
    GS=as.numeric(cor(as.data.frame(design)[,5],datExpr, use="p"))
...
if(T){
    ...
    if(T){
        Buffer_24h = as.data.frame(design[,5])
        names(Buffer_24h) = "Buffer_24h"
        # 得到矩阵
        geneTraitSignificance <-  as.data.frame(cor(datExpr, Buffer_24h, use = "p"))
        # 矩阵t检验
        GSPvalue <-  as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples))
        # 修改列名
        names(geneTraitSignificance) = paste("GS.", names(Buffer_24h), sep="")
        names(GSPvalue) = paste("p.GS.", names(Buffer_24h), sep="")
    }
...
if(T){
    ...
    goID <- bitr(modProbes, 'FLYBASE', "ENTREZID", org.Dm.eg.db)
...
#KEGG
if(T){
    keggID <- bitr(modProbes, 'FLYBASE', c("ENTREZID", "UNIPROT"),org.Dm.eg.db)
...
if(T){
    ...
    subGenes <- bitr(colnames(subTOM),fromType = "FLYBASE", toType = c("SYMBOL","ENTREZID","UNIPROT"),
                     OrgDb = org.Dm.eg.db)
...
write.csv(datTraits, "sample_metadata.csv")
write.csv(a, "count_table.csv")
